...
from model.TMPP import TMPP
from model.patchTST import PatchTST
from model.Informer import Informer
from model.Non_AR import Non_AR
from model.Fixed_Memory import Fixed_Memory
...
def get_model():
    if exp_model == 'MLP':
        ...
        return TMPP(hist_len, pred_len, in_dim, city_num, batch_size, device)
    elif exp_model == 'patchTST':
        return TMPP(hist_len, pred_len, in_dim, city_num, batch_size, device)
    elif exp_model == 'Informer':
        return TMPP(hist_len, pred_len, in_dim, city_num, batch_size, device)
    elif exp_model == 'Non_AR':
        return TMPP(hist_len, pred_len, in_dim, city_num, batch_size, device)
    elif exp_model == 'Fixed_Memory':
        return TMPP(hist_len, pred_len, in_dim, city_num, batch_size, device)
    else:
